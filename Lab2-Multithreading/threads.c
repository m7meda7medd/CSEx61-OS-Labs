...
#include <sys/time.h>
#include <time.h>
typedef struct Matrix_struct{int** matrix ;int row ;int col;}Matrix_t;

typedef struct Matrices{Matrix_t * m1 ,*m2 , *res ;int id ;int row_id; int col_id; }Param_t;
...
perror("Add arguments as Mat1_filename Mat_2filename  result_filename\n") ;
exit(1) ;
...
if (pthread_create(&MxM_thread,NULL,Multiply_Two_Matrices,(void*)&(arg)))
{
...
{
gettimeofday(&start, NULL); //start checking time
pthread_t* threads = (pthread_t*) malloc (sizeof(pthread_t)* matrix1.row) ;
Param_t arg   = {.m1=&matrix1,.m2=&matrix2, .res=&result, .row_id = matrix1.row};

 for(int t=0; t<matrix1.row; t++){
 err = pthread_create(&threads[t], NULL,Multiply_Row_by_Matrix
 , (void *)&arg);
 if (err){
...
gettimeofday(&start, NULL); //start checking time
pthread_t* threads = (pthread_t*) malloc (sizeof(pthread_t)* (result.row * result.col)) ;
Param_t arg   = {.m1=&matrix1,.m2=&matrix2, .res=&result, .row_id = matrix1.row-1, .col_id =matrix2.col};

 for(int t=0; t < (matrix1.row * matrix2.col ); t++){
 err = pthread_create(&threads[t], NULL,Multiply_Row_by_Col
 , (void *)&arg);
 if (err){
 perror("ERROR :  pthread_create()\n");
 exit(-1);
...
if (dup2(saved_fd,stdout_fd) == -1) {perror("Error with dup2 ");}
}
    ...
    for (int i = 0; i < Data->m1->row; i++) {
        for (int j = 0; j < Data->m2->col; j++) {
            ...
            for (int k = 0; k < Data->m1->col; k++) {
                Data->res->matrix[i][j] += Data->m1->matrix[i][k] * Data->m2->matrix[k][j];
            }
        }
    }
return NULL ;
...
Data->row_id-- ;
    // Perform matrix multiplication
        for (int j = 0; j < Data->m2->col; j++) {
            ...
            for (int k = 0; k < Data->m1->col; k++) {
                Data->res->matrix[Data->row_id][j] += Data->m1->matrix[Data->row_id][k] * Data->m2->matrix[k][j];
            }
        }
return NULL ;
}
...
Data->col_id--;
    // Perform matrix multiplication
            ...
            for (int k = 0; k < Data->m1->col; k++) {
                Data->res->matrix[Data->row_id][Data->col_id] += Data->m1->matrix[Data->row_id][k] * Data->m2->matrix[k][Data->col_id];
            }
return NULL ;
}
...
if ((row != 0) && (col != 0))
{
matrix_1->matrix = (int **) realloc (matrix_1->matrix ,sizeof(int*) * row) ;
...
for (int m = 0 ; m<row ; m++) {
matrix_1->matrix[m] = (int*)  realloc (matrix_1->matrix[m] , sizeof(int) * col ) ;
...
l_filename  = (char*)realloc(l_filename ,(strlen(file_name)+strlen(".txt")+1)) ;
if ((l_filename != NULL))
{
